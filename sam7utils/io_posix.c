...
//#define SAM7_TTY "/dev/ttyUSB0"
#define SAM7_TTY "/dev/ttyACM0"
...
int io_init( char *dev )
{
  ...
  if(tcgetattr(io_fd, &port_setting)!=0)
    return -2;
	
  port_setting.c_iflag = IGNBRK | IGNPAR;
  port_setting.c_cflag = B115200 | CS8 | CREAD | HUPCL | CLOCAL;
  port_setting.c_oflag = 0;
  port_setting.c_lflag = 0;
  
  if(tcsetattr(io_fd, TCSAFLUSH, &port_setting)!=0)
    return -3;
  else
     return samba_init();
}
