...
static char arch_buffer[32];
int samba_interactive;
...
const char *at91_arch_str( int id ) {
  ...
  sprintf(arch_buffer,"unkown id [0x%02X]",id);
...
int samba_init( void ) 
{
  ...
  /* force samba into a known state */
  if( io_send_cmd( "T#", &response, sizeof( response ) ) < 0 ) {
...
int samba_read_half_word( uint32_t addr, uint16_t *value )
{
  ...
  snprintf( cmd, 64, "h%08X,2#", (unsigned int) addr );
...
void dump_io(char *name, char *buf, int buflen)
{
  int i, col;

  if (1) return;

  printf("%s (%2d): ", name, buflen);
